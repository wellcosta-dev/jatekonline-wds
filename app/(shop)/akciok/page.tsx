...
function filterAndSort(allProducts: Product[], filters: FilterState): Product[] {
  ...
  switch (filters.sortBy) {
    ...
    default:
      result.sort((a, b) => {
        const discA = a.salePrice ? Math.round(((a.price - a.salePrice) / a.price) * 100) : 0;
        const discB = b.salePrice ? Math.round(((b.price - b.salePrice) / b.price) * 100) : 0;
        return discB - discA;
      });
  }
...
export default function AkciokPage() {
  ...
  const saleProducts = useMemo(
    () => products.filter((product) => product.salePrice != null && product.salePrice < product.price),
    [products]
  );

  const priceBounds: [number, number] = useMemo(() => {
    if (saleProducts.length === 0) return [0, 100000];
    const prices = saleProducts.map((p) => p.salePrice ?? p.price);
  ...
  const filteredProducts = useMemo(
    () => filterAndSort(saleProducts, filters),
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/80 text-sm mt-1">
              {saleProducts.length} termék kedvezményes áron
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-8">
        ...
        <main className="flex-1 min-w-0">
          ...
          <ProductGrid
            ...
            listName="Akciós termékek"
          />
          {loadingProducts ? (
            <p className="mt-4 text-sm text-neutral-medium">Akciós termékek betöltése...</p>
          ) : null}
        </main>
      </div>
    </div>
